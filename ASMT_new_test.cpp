...
void init()
{
	...
	// 		if (trees[0][j].vertexId == i)
	// 			cout << trees[0][j].vertexId << ' ' << trees[0][j].pathId << ' ' << printNode(0, j) << endl;
	...
	// 	for (int j = 0; j < trees[1].size(); j++)
	// 		if (trees[1][j].vertexId == i)
	// 			cout << trees[1][j].vertexId << ' ' << trees[1][j].pathId << ' ' << printNode(1, j) << endl;
